...
namespace RB.Genetix
{
    [Flags]
    public enum LandscapeProperty
    {
        None = 0,
        /// <summary>
        /// Плоский ландшафт, без вертикальных элементов, будь то скалы или деревья
        /// </summary>
        Flat = 1,
        /// <summary>
    ...
    {
        LandscapeProperty LandProperties { get; }
    }

    public class Fenotype : GenetixBase
    {
        public readonly static Fenotype s_HumanEtalon = new Fenotype(BodyGenetix.Human,
                                                HeadGenetix.Human,
                                                LegsGenetix.Human,
                                                ArmsGenetix.Human,
                                                WingsGenetix.None,
        ...
        /// </summary>
        /// <returns></returns>
        public string GetComparsion(Fenotype pOriginal)
        {
        ...
        /// <returns></returns>
        private ITerritory[] GetLands(ITerritory[] aLandTypes, LandscapeProperty eAllowedProp, LandscapeProperty eForbiddenProp)
        {
            List<ITerritory> cResult = new List<ITerritory>();
            for (int i = 0; i < aLandTypes.Length; i++)
            {
                if ((eAllowedProp == LandscapeProperty.None || aLandTypes[i].LandProperties.HasFlag(eAllowedProp)) && 
                    (eForbiddenProp == LandscapeProperty.None || !aLandTypes[i].LandProperties.HasFlag(eForbiddenProp)))
                    cResult.Add(aLandTypes[i]);
        ...
        public void GetTerritoryPreferences(ITerritory[] aAllLands, out ITerritory[] aPreferred, out ITerritory[] aHated)
        {
            Dictionary<ITerritory, int> cLandTypes = new Dictionary<ITerritory, int>();
               ...
               m_pLegs.m_eLegsCount != LegsCount.NoneTail)
            {
                switch (m_pLegs.m_eLegsType)
                {
                    ...
                    case LegsType.Hoofs:
                        foreach(ITerritory pLand in GetLands(aAllLands, LandscapeProperty.Open, LandscapeProperty.Soft))
                    ...
                    case LegsType.Paws:
                        foreach(ITerritory pLand in GetLands(aAllLands, LandscapeProperty.None, LandscapeProperty.Soft))
                            cLandTypes[pLand] += iMultiplier;
                        break;
                    //птичьи лапы с когтями - в лесах и в горах
                    case LegsType.Claws:
                        foreach (ITerritory pLand in GetLands(aAllLands, LandscapeProperty.None, LandscapeProperty.Flat))
                            cLandTypes[pLand] += iMultiplier;
                        break;
                    //паучьи лапы - в песках и горах
                    case LegsType.Spidery:
                        foreach(ITerritory pLand in GetLands(aAllLands, LandscapeProperty.None, LandscapeProperty.Wet))
                            cLandTypes[pLand] += iMultiplier;
                        break;
                    //щупальца - в болотах
                    case LegsType.Tentacles:
                        foreach(ITerritory pLand in GetLands(aAllLands, LandscapeProperty.Soft | LandscapeProperty.Wet, LandscapeProperty.Cold))
                            cLandTypes[pLand] += iMultiplier;
                        break;
                }
            }
            else
            {
                ...
                foreach (ITerritory pLand in GetLands(aAllLands, LandscapeProperty.Soft, LandscapeProperty.None))
                    cLandTypes[pLand] += iMultiplier;
            }
            ...
            if (m_pTail.m_eTailLength == TailLength.Long)
            {
                switch (m_pTail.m_eTailControl)
                {
                    ...
                    case TailControl.Skillful:
                        foreach(ITerritory pLand in GetLands(aAllLands, LandscapeProperty.None, LandscapeProperty.Flat | LandscapeProperty.Open))
                            cLandTypes[pLand] *= 2;
                        break;
                }
            }
            ...
            if (m_pWings.m_eWingsForce != WingsForce.None)
            {
                ...
                if (m_pWings.m_eWingsType == WingsType.Feathered)
                    foreach (ITerritory pLand in GetLands(aAllLands, LandscapeProperty.Soft | LandscapeProperty.Wet, LandscapeProperty.None))
                        cLandTypes[pLand] = 0;
            }
            ...
            switch (m_pHide.m_eHideType)
            {
                ...
                case HideType.FurLong:
                    foreach (ITerritory pLand in GetLands(aAllLands, LandscapeProperty.Cold, LandscapeProperty.None))
                        cLandTypes[pLand] *= 2;

                    foreach (ITerritory pLand in GetLands(aAllLands, LandscapeProperty.Hot, LandscapeProperty.None))
                        cLandTypes[pLand] = 0;
                    foreach (ITerritory pLand in GetLands(aAllLands, LandscapeProperty.Wet, LandscapeProperty.None))
                        cLandTypes[pLand] = 0;
                    break;
                ...
                case HideType.Shell:
                    foreach (ITerritory pLand in GetLands(aAllLands, LandscapeProperty.Wet | LandscapeProperty.Hot, LandscapeProperty.None))
                        cLandTypes[pLand] *= 2;
            ...
            {
                foreach (ITerritory pLand in GetLands(aAllLands, LandscapeProperty.None, LandscapeProperty.Open))
                    cLandTypes[pLand] *= 2;

                foreach (ITerritory pLand in GetLands(aAllLands, LandscapeProperty.Open, LandscapeProperty.Flat))
                    cLandTypes[pLand] = 0;
            }
            ...
            foreach (ITerritory pLand in GetLands(aAllLands, LandscapeProperty.None, LandscapeProperty.Habitable))
                cLandTypes[pLand] = 0;
            ...
            List<ITerritory> cPreferred = new List<ITerritory>();
            List<ITerritory> cHated = new List<ITerritory>();

            foreach (var pLand in cLandTypes)
            {
                if (pLand.Value == iMax)
                    cPreferred.Add(pLand.Key);
                if (pLand.Value == 0 && pLand.Key.LandProperties.HasFlag(LandscapeProperty.Habitable))
                    cHated.Add(pLand.Key);
            }
        ...
        {
            Fenotype pAnother = pOther as Fenotype;
