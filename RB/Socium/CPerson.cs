...
namespace RB.Socium
{
    public class CPerson
    {
        ...
        public string WhoAmITo(CPerson pOther)
        {
        ...
        public static void GetSkillPreferences(Culture pCulture, int iCultureLevel, Customs pCustoms, ref Skill eMostRespectedSkill, ref Skill eLeastRespectedSkill)
        {
            ...
            if (pCulture.MentalityValues[Mentality.Agression][iCultureLevel] > 1)
                cSkillPreferences[CPerson.Skill.Body]++;
            ...
            if (pCustoms.m_eSexuality == Customs.Sexuality.Puritan)
                cSkillPreferences[CPerson.Skill.Charm]--;
            ...
            eMostRespectedSkill = CPerson.Skill.Body;
            eLeastRespectedSkill = CPerson.Skill.Charm;

            int iMax = int.MinValue;
            int iMin = int.MaxValue;

            foreach (CPerson.Skill eSkill in Enum.GetValues(typeof(CPerson.Skill)))
            {
        ...
        {
            switch (m_pHomeLocation.Owner.m_pCustoms.m_eSexRelations)
            {
                ...
                case Customs.SexualOrientation.Bisexual:
                    ...
                    if (m_pHomeLocation.Owner.m_pCustoms.m_eGenderPriority == Customs.GenderPriority.Patriarchy &&
                    ...
                    if (m_pHomeLocation.Owner.m_pCustoms.m_eGenderPriority == Customs.GenderPriority.Matriarchy &&
        ...
        public Dictionary<CPerson, Relation> m_cRelations = new Dictionary<CPerson, Relation>();
        public CPerson m_pPatron = null;
        public List<CPerson> m_cMinions = new List<CPerson>();

        public int GetFactionSize()
        {
            int iCount = 1;

            foreach (CPerson pMinion in m_cMinions)
        ...
        public CPerson GetFaction()
        {
            int iDepth;
            return GetFaction(out iDepth);
        }

        public CPerson GetFaction(out int iDepth)
        {
            iDepth = 0;
        ...
        public CLocation m_pHomeLocation;
        public CNation m_pNation;
        ...
        private void InitSocial(CPerson pRelative)
        {
            m_pCulture = new Culture(pRelative == null ? m_pEstate.m_pCulture : pRelative.m_pCulture);
            m_iCultureLevel = pRelative == null ? m_pEstate.m_iCultureLevel : pRelative.m_iCultureLevel;
            ...
            if (m_iCultureLevel < m_pHomeLocation.Owner.m_iCultureLevel - 1)
                m_iCultureLevel = m_pHomeLocation.Owner.m_iCultureLevel - 1;
        ...
        public bool CouldInviteNewDwellers()
        {
            ...
            if (m_pBuilding.m_pInfo.m_eOwnership == FamilyOwnership.Owners && m_pProfession != m_pBuilding.m_pInfo.m_pOwner)
                return true;

            List<CPerson> cBloodRelatives = new List<CPerson>();
        ...
        public CPerson(CWorld pWorld, CLocation pHomeLocation, Building pBuilding, bool bOwner)
        {
            ...
            CPerson pRelative = null;

            if (m_pBuilding.m_cPersons.Count > 0)
            {
                List<CPerson> cRightfullOwners = new List<CPerson>();
                ...
                foreach (CPerson pDweller in m_pBuilding.m_cPersons)
                {
                    if ((bOwner && pDweller.m_pProfession == m_pBuilding.m_pInfo.m_pOwner) ||
            ...
            if (pRelative != null)
            {
                ...
                if (pRelative.m_pProfession == m_pBuilding.m_pInfo.m_pOwner)
                {
                    if (bOwner)
                ...
                {
                    CPerson pSpouse = pRelative.GetSpouse();
                    if (pSpouse == null)
                    {
                        eChoice = Relation.Sibling;
        ...
        /// <returns></returns>
        private Building FindSuitableBuilding(CPerson pRelative)
        {
            ...
            foreach (Building pBuilding in m_pHomeLocation.Settlement.m_cBuildings)
            {
                ...
                if (m_pEstate.m_cStratums.Contains(pOwner))
                {
                ...
                if (m_pEstate.m_cStratums.Contains(pWorkers))
                {
        ...
        public CPerson(CWorld pWorld, CLocation pPreferredHome)
        {
            CPerson pRelative = null;
            ...
            if (pRelative != null)
            {
                ...
                if (pRelative.m_pBuilding == m_pBuilding)
                {
                    ...
                    if (m_cRelations[pRelative] == Relation.ChildOf &&
                        ...
                        pRelative.m_pProfession == pHomeWorkers.m_pProfession)
                    {
                    ...
                    if (m_cRelations[pRelative] == Relation.ParentOf &&
                        m_pProfession == pHomeWorkers.m_pProfession &&
                        pRelative.m_pProfession == pHomeOwner.m_pProfession)
                    {
            ...
            if (m_pProfession == pHomeOwner.m_pProfession)
            {
                List<CPerson> cOwners = new List<CPerson>();
                foreach (CPerson pNeighbour in m_pBuilding.m_cPersons)
                {
                    if (pNeighbour.m_pProfession == pHomeOwner.m_pProfession)
        ...
        /// <returns></returns>
        private bool CheckGender(CPerson pRelative)
        {
            if (pRelative != null)
            {
                if (m_cRelations[pRelative] == Relation.Spouse ||
        ...
        {
            Dictionary<CPerson, Relation> cNewKins = new Dictionary<CPerson, Relation>();
        ...
        public void AddDrama()
        {
            ...
            List<CPerson> cNewRelatives = new List<CPerson>();
            ...
            {
                CPerson pNewKin = cNewRelatives[Rnd.Get(cNewRelatives.Count)];
        ...
        {
            Dictionary<CPerson, float> cTensions = new Dictionary<CPerson, float>();
            ...
            Dictionary<CPerson, float> cEnemies = new Dictionary<CPerson, float>();
            Dictionary<CPerson, float> cFriends = new Dictionary<CPerson, float>();
            Dictionary<CPerson, float> cLovers = new Dictionary<CPerson, float>();

            Dictionary<CPerson, Relation> cChanges = new Dictionary<CPerson,Relation>();
            ...
            foreach (var pRelative in m_cRelations)
            {
                ...
                if (fTension > 0)
                {
                        ...
                        m_eGender == pRelative.Key.GetCompatibleSexPartnerGender(false))
                    {
                        ...
                        {
                            switch (pRelative.Key.m_pCustoms.m_eSexuality)
                            {
            ...
            if (cLovers.Count > 0)
            {
                CPerson pLover = cLovers.Keys.ElementAt(Rnd.ChooseOne(cLovers.Values, 3));
           ...
           if (cEnemies.Count > 0)
            {
                //if (iEnemies == 0)
                {
                    CPerson pEnemy = cEnemies.Keys.ElementAt(Rnd.ChooseOne(cEnemies.Values, 3));
            ...
            if (cFriends.Count > 0)
            {
                if (iFriends == 0)
                {
                    CPerson pFriend = cFriends.Keys.ElementAt(Rnd.ChooseOne(cFriends.Values, 3));
        ...
        /// <returns></returns>
        private Relation PossibleRelation(CPerson pPretender)
        {
        ...
        public int GetFenotypeDifference(CPerson pOpponent, ref string sPositiveReasons, ref string sNegativeReasons)
        {
            int iHostility = 0;
        ...
        /// <returns></returns>
        public int GetCustomsDifference(CPerson pOpponent, ref string sPositiveReasons, ref string sNegativeReasons)
        {
            int iHostility = 0;

            Customs pMine = m_pCustoms;
            ...
            if (m_pEstate.IsSegregated() && m_eGender != pOpponent.m_eGender)
                pMine = Customs.Merge(m_pCustoms, m_pEstate.m_pCustoms, m_pEstate.GetCustoms(pOpponent.m_eGender));
            Customs pOpponents = pOpponent.m_pCustoms;
        ...
        public string GetFenotypeDescription()
        {
            ...
            if (sResult.Length == 0)
                sResult = "is just a common " + m_eAge.ToString().ToLower() + " " + (m_eGender == CPerson._Gender.Male ? "man" : "woman") + ".";

            if (!sResult.StartsWith("is"))
                sResult = "is a quite common " + m_eAge.ToString().ToLower() + " " + (m_eGender == CPerson._Gender.Male ? "man" : "woman") + ". " + sResult;
        ...
        public string GetFenotypeComparsion(Fenotype pOriginal)
        {
                ...
                !pOriginal.m_pTail.IsIdentical(m_pFenotype.m_pTail))
            {
                ...
                sResult += m_eGender == CPerson._Gender.Male ? "He has " : "She has ";
                ...
                !pOriginal.m_pHide.IsIdentical(m_pFenotype.m_pHide))
            {
                ...
                if (!pOriginal.m_pBody.IsIdentical(m_pFenotype.m_pBody))
                {
                    sResult += (m_eGender == CPerson._Gender.Male ? "He is a " : "She is a ") + m_pFenotype.m_pBody.GetDescription(m_eGender);
                    bSemicolon = true;
                }
                else
                    sResult += (m_eGender == CPerson._Gender.Male ? "He " : "She ") + "has ";
                ...
                (pOriginal.m_pLifeCycle.m_eDyingRate == DyingRate.Moderate && m_eAge > _Age.Aged))
            {
                ...
                sResult += (m_eGender == CPerson._Gender.Male ? "He" : "She") + " looks very old for " + (m_eGender == CPerson._Gender.Male ? "his" : "her") + " age.";
            }
            ...
            if ((pOriginal.m_pLifeCycle.m_eDyingRate == DyingRate.Low && m_eAge > _Age.Adult))
            {
                ...
                sResult += (m_eGender == CPerson._Gender.Male ? "He" : "She") + " looks surprisingly young for " + (m_eGender == CPerson._Gender.Male ? "his" : "her") + " age.";
            }
        ...
        public string GetCustomsDescription()
        {
            ...
            if (sResult != "" && m_pCustoms.m_eMindSet != Customs.MindSet.Balanced_mind)
                sResult += ", ";

            if (m_pCustoms.m_eMindSet == Customs.MindSet.Emotions)
                sResult += "usually behaves " + (m_eGender == CPerson._Gender.Male ? "himself" : "herself") + " very emotinally";
            if (m_pCustoms.m_eMindSet == Customs.MindSet.Logic)
                sResult += "usually behaves " + (m_eGender == CPerson._Gender.Male ? "himself" : "herself") + " quite unemotional";
            //if (m_pCustoms.m_eMindSet == Customs.MindSet.Balanced_mind)
            //    sResult += "combines emotions and logic";

            if (sResult != "" && m_pCustoms.m_eMagic != Customs.Magic.Magic_Allowed)
                sResult += ", ";

            if (m_pCustoms.m_eMagic == Customs.Magic.Magic_Feared)
                sResult += "shows a lot of fear about magic";
            if (m_pCustoms.m_eMagic == Customs.Magic.Magic_Praised)
            ...
            if (sResult != "" && m_pCustoms.m_eProgress != Customs.Progressiveness.Moderate_Science)
                sResult += ", ";
            ...
            if (sResult != "" && (m_pCustoms.m_eBodyModifications == Customs.BodyModifications.Body_Modifications_Blamed || m_pCustoms.m_cMandatoryModifications.Count > 0))
                sResult += ", ";

            if (m_pCustoms.m_eBodyModifications == Customs.BodyModifications.Body_Modifications_Blamed)
                sResult += "rejects any form of body modification";
            else
            {
                bool bFirst2 = true;
                foreach (Customs.BodyModificationsTypes eMod in m_pCustoms.m_cMandatoryModifications)
                {
                    if (bFirst2)
                        sResult += "has ";
                    else
            ...
            if (m_pCustoms.m_eClothes == Customs.Clothes.Covering_Clothes)
                sResult += "wears hiding clothes";
            if (m_pCustoms.m_eClothes == Customs.Clothes.Minimal_Clothes)
                sResult += "wears quite revealing clothes";
            if (m_pCustoms.m_eClothes == Customs.Clothes.Revealing_Clothes)
                sResult += "wears common clothes";

            if (sResult != "" && m_pCustoms.m_eAdornments != Customs.Adornments.No_Adornments)
                sResult += " with ";

            //if (m_pCustoms.m_eAdornments == Customs.Adornments.No_Adornments)
            //    sResult += "wears no adornments";
            if (m_pCustoms.m_eAdornments == Customs.Adornments.Lavish_Adornments)
                sResult += "a lot of adornments";
            if (m_pCustoms.m_eAdornments == Customs.Adornments.Some_Adornments)
                sResult += "some adornments";

            if (sResult != "" && m_pCustoms.m_eFamilyValues != Customs.FamilyValues.Moderate_Family_Values)
                sResult += ", ";

            if (m_pCustoms.m_eFamilyValues == Customs.FamilyValues.Praised_Family_Values)
                sResult += "have very tight family bonds";
            if (m_pCustoms.m_eFamilyValues == Customs.FamilyValues.No_Family_Values)
            ...
            sResult += m_eGender == CPerson._Gender.Male ? "He is " : "She is ";
            ...
            else
            {
                ...
                if (m_pCustoms.m_eSexuality == Customs.Sexuality.Moderate_sexuality)
                    sResult += "a moderate ";

                if (m_pCustoms.m_eSexuality != Customs.Sexuality.Puritan)
                {
                    if (m_pCustoms.m_eSexRelations == Customs.SexualOrientation.Heterosexual)
                        sResult += m_eGender == CPerson._Gender.Male ? "stright male" : "strignt female";
                    if (m_pCustoms.m_eSexRelations == Customs.SexualOrientation.Homosexual)
                        sResult += m_eGender == CPerson._Gender.Male ? "gay" : "lesbian";
                    if (m_pCustoms.m_eSexRelations == Customs.SexualOrientation.Bisexual)
                        sResult += m_eGender == CPerson._Gender.Male ? "besexual male" : "besexual female";
                }
            }

            sResult = (m_eGender == CPerson._Gender.Male ? "He " : "She ") + sResult + ".";
        ...
        public string GetDescription()
        {
            ...
            if (pInjuries.Length > 0)
                pResult.AppendLine((m_eGender == CPerson._Gender.Male ? "He has " : "She has ") + pInjuries.ToString());
            ...
            if (pDisorders.Length > 0)
                pResult.AppendLine((m_eGender == CPerson._Gender.Male ? "His" : "Her") + " most remarkable " +
                    (m_cMentalDisorder.Count > 1 ? "flaws are " : "flaw is ") + pDisorders.ToString() + ".");


            if (m_pProfession != null)
            {
                pResult.AppendFormat("{0} is a {1}, one of {2}s estate.\n{0} is ", m_eGender == CPerson._Gender.Male ? "He" : "She", m_eGender == CPerson._Gender.Male ? m_pProfession.m_sNameM : m_pProfession.m_sNameF, m_pEstate.m_sName.ToLower());
                ...
                pResult.AppendFormat("{0} {1}.\n", pSkills.ToString(), m_eGender == CPerson._Gender.Male ? "man" : "woman");
            }
        ...
        /// <returns></returns>
        public float GetAttitudeTo(CPerson pOpponent)
        {
        ...
        public float GetFactionAttitudeTo(CPerson pOpponent)
        {
            float fProximity = GetProximity(this, pOpponent);
            float fHostility = -CalcNormalizedHostility(pOpponent, false);
        ...
        public static float GetAttraction(CPerson pPerson1, CPerson pPerson2)
        {
        ...
        public static float GetFactionAttraction(CPerson pPerson1, CPerson pPerson2)
        {
            //если взаимные отношения откладывать по осям x и y, то напряжение - это проекция полученной точки на ось x=y.
            //sin(45) = 0.70710...
        ...
        public float CalcNormalizedHostility(CPerson pOpponent, bool bFirstLook)
        {
            int iMinHostility = int.MaxValue;
            int iMaxHostility = int.MinValue;
            int iActualHostility = 0;

            Dictionary<CPerson, int> cHostility = new Dictionary<CPerson, int>();
            ...
            int iMedian = CMedian.Median(cHostility.Values.ToList());
            ...
            if (fHostility > 0)
                fHostility = (float)(m_pCulture.MentalityValues[Mentality.Fanaticism][m_iCultureLevel] * fHostility * fInfluenceDiff * fProximity);
            else if (fHostility < 0)
                fHostility = (float)((2.0f - m_pCulture.MentalityValues[Mentality.Fanaticism][m_iCultureLevel]) * fHostility * fInfluenceDiff * fProximity);
        ...
        /// <param name="pOpponent"></param>
        /// <returns></returns>
        public int CalcHostility(CPerson pOpponent, bool bFirstLook)
        {
            string sReasons;
        ...
        /// <param name="pOpponent"></param>
        /// <returns></returns>
        public int CalcHostility(CPerson pOpponent, out string sReasons, bool bFirstLook)
        {
            if (pOpponent == null)
            {
            ...
            {
                if(m_pCustoms.m_eSexuality == Customs.Sexuality.Lecherous ||
                    (m_pCustoms.m_eSexuality == Customs.Sexuality.Moderate_sexuality && 
                     (!m_cRelations.ContainsKey(pOpponent) || !IsBloodKinship(m_cRelations[pOpponent]))))
                {
                    if (pOpponent == this ||
                        m_pCustoms.m_eSexRelations == Customs.SexualOrientation.Bisexual ||
                        (m_pCustoms.m_eSexRelations == Customs.SexualOrientation.Heterosexual && pOpponent.m_eGender != m_eGender) ||
                        (m_pCustoms.m_eSexRelations == Customs.SexualOrientation.Homosexual && pOpponent.m_eGender == m_eGender))
                    {
                        int iBeauty = 0;
        ...
        /// </summary>
        /// <param name="pRelative"></param>
        private void FixSkills(CPerson pRelative)
        {
        ...
        public void FixHealth()
        {
            foreach (var pRelation in m_cRelations)
            {
                if (pRelation.Value == Relation.AlterEgo ||
                    pRelation.Value == Relation.PresentLifeOf)
                {
                    foreach (Customs.BodyModificationsTypes eMod in pRelation.Key.m_pCustoms.m_cMandatoryModifications)
                        if (!m_pCustoms.m_cMandatoryModifications.Contains(eMod))
                            m_pCustoms.m_cMandatoryModifications.Add(eMod);
        ...
        /// </summary>
        /// <param name="pPretender"></param>
        public void AddRelation(CPerson pPretender)
        {
        ...
        /// <returns></returns>
        public static float GetProximity(CPerson pPerson1, CPerson pPerson2)
        {
            string sResults;
        ...
        /// <param name="pPerson2"></param>
        /// <returns></returns>
        public static float GetProximity(CPerson pPerson1, CPerson pPerson2, out string sResults)
        {
        ...
        public int GetFamilySize()
        {
            ...
            List<CPerson> cFamily = new List<CPerson>();
            cFamily.Add(this);
            cProcessed.Add(this);
        ...
        /// </summary>
        /// <returns></returns>
        public CPerson[] GetSiblings()
        {
            List<CPerson> cProcessed = new List<CPerson>();
            List<CPerson> cSiblings = new List<CPerson>();
            cSiblings.Add(this);
            cProcessed.Add(this);
        ...
        public CPerson GetParent()
        {
            ...
            CPerson[] cSiblings = GetSiblings();
            foreach (CPerson pSibling in cSiblings)
            {
        ...
        /// </summary>
        /// <returns></returns>
        public CPerson GetSpouse()
        {
            switch (m_pCustoms.m_eMarriage)
            {
        ...
        private bool GetSignificantOther(Relation eRelation, ref CPerson pSignificantOther)
        {
            if (m_cRelations.ContainsValue(eRelation))
            {
                CPerson pPretender = GetHighestRelative(eRelation);
        ...
        public void SetFullName()
        {
            ...
            CPerson pSignificantOther = null;
        ...
        /// <returns></returns>
        private CPerson GetHighestRelative(Relation eRelation)
        {
            if (!m_cRelations.ContainsValue(eRelation))
                return null;

            List<CPerson> cRelatives = new List<CPerson>();
            ...
            CPerson pMostSignificant = cRelatives[0];
            int iMax = 0;

            foreach (CPerson pPerson in cRelatives)
            {
