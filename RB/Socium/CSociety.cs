...
namespace RB.Socium
{
    public abstract class CSociety
    {
        /// <summary>
        ...
        protected List<CLocation> m_cLands = new List<CLocation>();
        ...
        protected List<CLocation> m_cSettlements = new List<CLocation>();
        ...
        protected CPerson.Skill m_eMostRespectedSkill;

        /// <summary>
        /// Наиболее престижный навык. Задаётся в BuildCapital!
        /// </summary>
        public CPerson.Skill MostRespectedSkill
        {
            get { return m_eMostRespectedSkill; }
        }

        protected CPerson.Skill m_eLeastRespectedSkill;

        /// <summary>
        /// Наименее престижный навык. Задаётся в BuildCapital!
        /// </summary>
        public CPerson.Skill LeastRespectedSkill
        {
        ...
        public Dictionary<CStratum, int> m_cPeople = new Dictionary<CStratum, int>();
        ...
        /// <param name="pProfession"></param>
        /// <returns></returns>
        public CStratum GetStratum(ProfessionInfo pProfession)
        {
            ...
            return new CStratum(this, pProfession);
        }
        ...
        public void SetEstates()
        {
            ...
            foreach (CLocation pLocation in m_cLands)
            {
                ...
                if (pLocation.Settlement.m_cBuildings.Count > 0)
                {
                    foreach (Building pBuilding in pLocation.Settlement.m_cBuildings)
                    {
                        ...
                        CStratum pOwner = GetStratum(pBuilding.m_pInfo.m_pOwner);
                        m_cPeople.TryGetValue(pOwner, out iCount);
                        m_cPeople[pOwner] = iCount + iOwnersCount;

                        CStratum pWorkers = GetStratum(pBuilding.m_pInfo.m_pWorkers);
            ...
            SortedDictionary<int, List<CStratum>> cStratumPreference = new SortedDictionary<int, List<CStratum>>();
            ...
            foreach (var pStratum in m_cPeople)
            {
                ...
                if (!cStratumPreference.ContainsKey(iPreference))
                    cStratumPreference[iPreference] = new List<CStratum>();

                cStratumPreference[iPreference].Add(pStratum.Key);
            }
            ...
            foreach (var pStratum in m_cPeople)
            {
                if (pStratum.Key.m_pProfession.m_eCasteRestriction.HasValue)
                {
                    switch (pStratum.Key.m_pProfession.m_eCasteRestriction)
                    {
                        case ProfessionInfo.Caste.Elite:
                            m_cEstates[CEstate.Position.Elite].m_cStratums.Add(pStratum.Key);
                            iEliteEstateCount -= pStratum.Value;
                            RemoveStratumPreference(ref cStratumPreference, pStratum.Key);
                            break;
                        case ProfessionInfo.Caste.Low:
                            m_cEstates[CEstate.Position.Low].m_cStratums.Add(pStratum.Key);
                            iLowEstateCount -= pStratum.Value;
                            RemoveStratumPreference(ref cStratumPreference, pStratum.Key);
                            break;
                        case ProfessionInfo.Caste.Outlaw:
                            m_cEstates[CEstate.Position.Outlaw].m_cStratums.Add(pStratum.Key);
                            RemoveStratumPreference(ref cStratumPreference, pStratum.Key);
                            break;
                    }
                }
            ...
            if (m_iSocialEquality != 0 || m_cEstates[CEstate.Position.Low].m_cStratums.Count == 0)
            {
                while (iLowEstateCount > 0)
                {
                    if (cStratumPreference.Count == 0)
                        break;

                    CStratum pBestFit = null;
                    int iLowestPreference = cStratumPreference.Keys.First();

                    int iBestFit = int.MinValue;
                    foreach (CStratum pStratum in cStratumPreference[iLowestPreference])
                    {
                        if (pStratum.m_pProfession.m_eCasteRestriction != ProfessionInfo.Caste.MiddleOrUp &&
                            m_cPeople[pStratum] < iLowEstateCount && m_cPeople[pStratum] > iBestFit)
                        {
                            pBestFit = pStratum;
                            iBestFit = m_cPeople[pStratum];
                        }
                    }
                    if (pBestFit == null)
                    {
                    ...
                    if (pBestFit != null)
                    {
                        m_cEstates[CEstate.Position.Low].m_cStratums.Add(pBestFit);
                        iLowEstateCount -= iBestFit;
                        RemoveStratumPreference(ref cStratumPreference, pBestFit);
                    }
                    else
                    {
                        break;
            ...
            while (iEliteEstateCount > 0)
            {
                ...
                CStratum pBestFit = null;
                int iHighestPreference = cStratumPreference.Keys.Last();

                int iBestFit = iEliteEstateCount;
                foreach (CStratum pStratum in cStratumPreference[iHighestPreference])
                {
                    if (m_cPeople[pStratum] < iBestFit)
                    {
                        pBestFit = pStratum;
                        iBestFit = m_cPeople[pStratum];
                    }
                ...
                if (pBestFit != null)
                {
                    m_cEstates[CEstate.Position.Elite].m_cStratums.Add(pBestFit);
            ...
            foreach (var pPreference in cStratumPreference)
            {
                foreach (CStratum pStratum in pPreference.Value)
                {
                    m_cEstates[CEstate.Position.Middle].m_cStratums.Add(pStratum);
                }
            }
        ...
        /// </summary>
        /// <param name="cStratumPreference"></param>
        /// <param name="pStratum"></param>
        private void RemoveStratumPreference(ref SortedDictionary<int, List<CStratum>> cStratumPreference, CStratum pStratum)
        {
            List<int> cCemetary = new List<int>();
            foreach (var pPreference in cStratumPreference)
            {
                if (pPreference.Value.Contains(pStratum))
                {
                    pPreference.Value.Remove(pStratum);
            ...
            foreach (int iPreference in cCemetary)
                cStratumPreference.Remove(iPreference);
        }

        public abstract string GetEstateName(CEstate.Position ePosition);

        protected abstract BuildingInfo ChooseNewBuilding(CSettlement pSettlement);

        public abstract void AddBuildings(CSettlement pSettlement);
    }
}
