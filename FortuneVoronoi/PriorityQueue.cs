	...
	public class BinaryPriorityQueue : IPriorityQueue, ICollection, ICloneable, IList
	{
		...
		public object Pop()
		{
			...
			do
			{
				...
				p1 = 2*p+1;
				p2 = 2*p+2;
                if (InnerList.Count > p1 && Comparer.Compare(InnerList[p], InnerList[p1]) > 0) // links kleiner
					p = p1;
                if (InnerList.Count > p2 && Comparer.Compare(InnerList[p], InnerList[p2]) > 0) // rechts noch kleiner
					p = p2;
