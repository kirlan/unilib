...
namespace Persona
{
    ...
    public class Event
    {
        ...
        public Event(UniLibXML pXml, XmlNode pParamNode, List<Action> cActions, List<Parameter> cParams, List<ObjectsCollection> cCollections, List<Flow> cFlows)
        {
            ...
            foreach (XmlNode pSubNode in pParamNode.ChildNodes)
            {
                ...
                if (pSubNode.Name == "Alternatives")
                {
                    foreach (XmlNode pSituationNode in pSubNode.ChildNodes)
                    {
                        ...
                        {
                            Situation pSituation = new Situation(pXml, pSituationNode, cParams, cCollections);
                ...
                if (pSubNode.Name == "Reactions")
                {
                    foreach (XmlNode pReactionNode in pSubNode.ChildNodes)
                    {
                        ...
                        {
                            Reaction pReaction = new Reaction(pXml, pReactionNode, cParams, cCollections, cFlows);
        ...
        internal void PreReaction(Module pModule)
        {
            pModule.m_sLog.AppendLine("PREREACTION " + m_sID);
            foreach (Consequence pConsequence in m_cConsequences)
