	...
	public class VistaButton : System.Windows.Forms.UserControl
	{
		...
		#region -  Properties  -
			...
			#region -  Appearance  -
                ...
                private byte mAlpha = 127;
                /// <summary>
                /// Level of transparency
                /// </summary>
                [Category("Appearance"),
                 DefaultValue(typeof(byte), "127"),
                 Description("The transparency level of bottom" +
                             "part of button. Will work only if " +
                             "BaseColor is set to Transparent.")]
                public byte Alpha
                {
                    get { return (byte)(byte.MaxValue - mAlpha); }
                    set { mAlpha = (byte)(byte.MaxValue - value); this.Invalidate(); }
                }
		...
		#region -  Drawing  -
			...
			{
				if (this.ButtonStyle == Style.Flat && this.mButtonState == State.None){return;}
                int alpha = (mButtonState == State.Pressed) ? 255 - mAlpha/2 : mAlpha;
				Rectangle r = this.ClientRectangle;
				r.Width--; r.Height--;
				using (GraphicsPath rr = RoundRect(r, CornerRadius, CornerRadius, CornerRadius, CornerRadius))
				{
			...
			{
				if (this.ButtonStyle == Style.Flat && this.mButtonState == State.None){return;}
                int alpha = (mButtonState == State.Pressed) ? mAlpha/2 : mAlpha;
				...
				using (GraphicsPath r = RoundRect(rect, CornerRadius, CornerRadius, 0, 0))
				{
					using (LinearGradientBrush lg = new LinearGradientBrush(r.GetBounds(), 
												...
												Color.FromArgb(alpha / 3, this.ButtonColor), 
...
//                    glow.AddEllipse(-5, this.Height / 2 - 10, this.Width + 11, this.Height + 11);
                    glow.AddEllipse(-5, -5, this.Width + 10, this.Height + 10);
