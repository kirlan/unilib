...
namespace GridBuilderTest
{
        ...
        where LOC : Location, new()
    {
        ...
        public LOC[] m_aNorthHemisphereLocations = null;
        public LOC[] m_aSouthHemisphereLocations = null;
        ...
        public LocationsGrid(int iLocations, int iWidth, int iHeight, string sDescription, WorldShape eShape)
        {
            ...
            do
            {
                if (eShape == WorldShape.Planet)
                {
                    ...
                    bOK = CreateSphereCH();
                    //BuildSphereRandomGrid();
                    //bOK = CalculateVoronoiSphere();
        ...
        /// <summary>
        /// Сворачивает 3 проекции цилиндра (экваториальный пояс и 2 "крышки") в сферу.
        /// Конечный список 3D-локаций (уже без "призраков") сохраняется в m_aLocations
        /// </summary>
        private void RestoreSphere()
        {
            float fTrueR = m_iRX / (float)Math.PI;

            List<LOC> cFinalLocs = new List<LOC>();
            foreach (LOC pLoc in m_aLocations)
            {
                if (pLoc.m_bGhost || pLoc.m_bUnclosed)
                    continue;

                float fLongitude = pLoc.m_fX / RX;
                float fLatitude = pLoc.m_fY / RY;

                pLoc.X = (float)((float)fTrueR * Math.Cos(fLatitude) * Math.Cos(fLongitude));
                pLoc.Y = (float)((float)fTrueR * Math.Cos(fLatitude) * Math.Sin(fLongitude));
                pLoc.Z = (float)((float)fTrueR * Math.Sin(fLatitude));

                cFinalLocs.Add(pLoc);
            }
            foreach (LOC pLoc in m_aNorthHemisphereLocations)
            {
                if (pLoc.m_bGhost || pLoc.m_bUnclosed)
                    continue;

                float fLongitude = (float)(Math.Atan2(pLoc.m_fY, pLoc.m_fX));
                float fLatitude = (float)(Math.PI / 2 - Math.Sqrt(pLoc.m_fX * pLoc.m_fX + pLoc.m_fY * pLoc.m_fY) / RY);
            ...
            foreach (LOC pLoc in m_aSouthHemisphereLocations)
            {
                if (pLoc.m_bGhost || pLoc.m_bUnclosed)
                    continue;

                float fLongitude = (float)(Math.Atan2(pLoc.m_fY, pLoc.m_fX));
                float fLatitude = (float)(Math.Sqrt(pLoc.m_fX * pLoc.m_fX + pLoc.m_fY * pLoc.m_fY) / RY - Math.PI / 2);
            ...
            m_aLocations = cFinalLocs.ToArray();
        }

        /// <summary>
        /// Добавляет информацию о смежности локаций на сфере.
        /// Фактически строит триангуляцию Делоне. Диаграмму Вороного на сфере мы построим сами, позже, на основании триангуляции.
        /// Координаты локаций так же остаются неизменными - возвращать их из плоских проекций в пространство будем отдельной функцией.
        /// </summary>
        /// <param name="cData">словарь соответствия вершин из данных диаграммы вороного нашим локациям</param>
        /// <param name="pEdge">грань диаграммы вороного, разделяющая смежные локации</param>
        /// <returns></returns>
        private bool AddTriangulationLink(Dictionary<BTVector, LOC> cData, VoronoiEdge pEdge)
        {
            LOC pLoc1 = null;
            LOC pLoc2 = null;
            ...
            if ((pLoc1.m_pEquatorOrigin != null || pLoc1.m_pHemisphereOrigin != null) && 
                (pLoc2.m_pEquatorOrigin != null || pLoc2.m_pHemisphereOrigin != null))
                return false;
            ...
            if (pLoc1 != null && pLoc1.m_pEquatorOrigin == null && pLoc1.m_pHemisphereOrigin == null)
            {
                if (pLoc2 != null)
                {
                    if (pLoc2.m_pEquatorOrigin == null)
                    {
                        if(pLoc2.m_pHemisphereOrigin == null)
                            pLoc1.BorderWith[pLoc2] = new List<Line>();
                        else
                            pLoc1.BorderWith[(LOC)pLoc2.m_pHemisphereOrigin] = new List<Line>();
                    }
                    else
                        pLoc1.BorderWith[(LOC)pLoc2.m_pEquatorOrigin] = new List<Line>();
                }
            }

            if (pLoc2 != null && pLoc2.m_pEquatorOrigin == null && pLoc2.m_pHemisphereOrigin == null)
            {
                if (pLoc1 != null)
                {
                    if (pLoc1.m_pEquatorOrigin == null)
                    {
                        if (pLoc1.m_pHemisphereOrigin == null)
                        {
                            pLoc2.BorderWith[pLoc1] = new List<Line>();
                        }
                        else
                            pLoc2.BorderWith[(LOC)pLoc1.m_pHemisphereOrigin] = new List<Line>();
                    }
                    else
                        pLoc2.BorderWith[(LOC)pLoc1.m_pEquatorOrigin] = new List<Line>();
                }
            }
        ...
        private bool AddEdge(Dictionary<BTVector, LOC> cData, Dictionary<BTVector, Vertex> cVertexes, List<Vertex> cMidPoints, VoronoiEdge pEdge)
        {
            ...
            //если обе локации "призрачные" - нас это не интересует
            if (pLoc1.m_pEquatorOrigin != null && pLoc2.m_pEquatorOrigin != null)
                return false;
            ...
            Vertex pLoc1Point = (pLoc1 != null && pLoc1.m_pEquatorOrigin == null) ? 
            ...
            Vertex pLoc2Point = (pLoc2 != null && pLoc2.m_pEquatorOrigin == null) ? 
            ...
            if (pLoc1 != null && pLoc1.m_pEquatorOrigin == null)
            {
                if (!pVertexA.m_cLocationsBuild.Contains(pLoc1))
                    pVertexA.m_cLocationsBuild.Add(pLoc1);
                if (!pVertexB.m_cLocationsBuild.Contains(pLoc1))
                ...
                if (pLoc2 != null)
                {
                    ...
                    if (pLine.m_fLength > 0)
                    {
                        ...
                        {
                            pLoc1.BorderWith[(LOC)pLoc2.m_pEquatorOrigin] = new List<Line>();
                            pLoc1.BorderWith[(LOC)pLoc2.m_pEquatorOrigin].Add(pLine);
                        }
                    }
                }
            }

            if (pLoc2 != null && pLoc2.m_pEquatorOrigin == null)
            {
                if (!pVertexA.m_cLocationsBuild.Contains(pLoc2))
                    pVertexA.m_cLocationsBuild.Add(pLoc2);
                if (!pVertexB.m_cLocationsBuild.Contains(pLoc2))
                ...
                if (pLoc1 != null)
                {
                    ...
                    if (pLine.m_fLength > 0)
                    {
                        ...
                        {
                            pLoc2.BorderWith[(LOC)pLoc1.m_pEquatorOrigin] = new List<Line>();
                            pLoc2.BorderWith[(LOC)pLoc1.m_pEquatorOrigin].Add(pLine);
                        }
                    }
                }
            }
            ...
            for (int yy = -iHeightReserv; yy < iHeight / 2 + iHeightReserv; yy++)
                for (int xx = -iWidthReserv; xx < iWidth + iWidthReserv; xx++)
                {
                    ...
                    if (m_eShape == WorldShape.Ringworld)
                    {
                        ...
                        if (stepX/2 + fx > RX)
                            pGhostLocation.Create(iID + iWidth * iHeight * 4, -RX * 3 + stepX/2 + fx, -RY + stepY / 2 + fy, 0, pLocation, null);
                        else
                            pGhostLocation.Create(iID + iWidth * iHeight * 4, RX + stepX / 2 + fx, -RY + stepY / 2 + fy, 0, pLocation, null);
                ...
                if (!pLoc.m_bBorder && !pLoc.m_bGhost)
                {
                    ...
                    if (iCount != 0)
                    {
                        ...
                        if (m_eShape == WorldShape.Ringworld || m_eShape == WorldShape.Planet)
                        {
                            ...
                            if (pImprovedLoc.X > 0)
                                pGhostLocation.Create(cLocations.Count, pImprovedLoc.X - RX * 2, pImprovedLoc.Y, 0, pImprovedLoc, null);
                            else
                                pGhostLocation.Create(cLocations.Count, pImprovedLoc.X + RX * 2, pImprovedLoc.Y, 0, pImprovedLoc, null);
        ...
        private List<LOC> BuildRandomGridFrame()
        {
            ...
            for (int ii = 0; ii <= kx; ii++)
            {
                ...
                if (m_eShape == WorldShape.Ringworld)
                {
                    ...
                    if (pLocation11.X > 0)
                        pLocation11Ghost.Create(cLocations.Count, pLocation11.X - RX * 2, pLocation11.Y, 0, pLocation11, null);
                    else
                        pLocation11Ghost.Create(cLocations.Count, pLocation11.X + RX * 2, pLocation11.Y, 0, pLocation11, null);
                    ...
                    if (pLocation12.X > 0)
                        pLocation12Ghost.Create(cLocations.Count, pLocation12.X - RX * 2, pLocation12.Y, 0, pLocation12, null);
                    else
                        pLocation12Ghost.Create(cLocations.Count, pLocation12.X + RX * 2, pLocation12.Y, 0, pLocation12, null);
                    ...
                    if (pLocation21.X > 0)
                        pLocation21Ghost.Create(cLocations.Count, pLocation21.X - RX * 2, pLocation21.Y, 0, pLocation21, null);
                    else
                        pLocation21Ghost.Create(cLocations.Count, pLocation21.X + RX * 2, pLocation21.Y, 0, pLocation21, null);
                    ...
                    if (pLocation22.X > 0)
                        pLocation22Ghost.Create(cLocations.Count, pLocation22.X - RX * 2, pLocation22.Y, 0, pLocation22, null);
                    else
                        pLocation22Ghost.Create(cLocations.Count, pLocation22.X + RX * 2, pLocation22.Y, 0, pLocation22, null);
        ...
        private void BuildPlainRandomGrid()
        {
            ...
            for (int i = 0; i < m_iLocationsCount; i++)
            {
                ...
                if (m_eShape == WorldShape.Ringworld)
                {
                    ...
                    if (pLocation.X > 0)
                        pGhostLocation.Create(cLocations.Count, pLocation.X - RX * 2, pLocation.Y, 0, pLocation, null);
                    else
                        pGhostLocation.Create(cLocations.Count, pLocation.X + RX * 2, pLocation.Y, 0, pLocation, null);
        ...
        struct SphereVertex
        {
            public float fX3d, fY3d, fZ3d;
        }

        private void BuildSphereRandomGrid()
        {
            List<LOC> cEquatorLocations = new List<LOC>();
            List<LOC> cNorthLocations = new List<LOC>();
            List<LOC> cSouthLocations = new List<LOC>();
            ...
            SphereVertex[] pSphere = new SphereVertex[m_iLocationsCount];

            for (int i = 0; i < m_iLocationsCount; i++)
            {
                SphereVertex pSV = new SphereVertex();

                float fD, fD2;

                do
                {
                    pSV.fX3d = RX - dkx * 2 - Rnd.Get(RX * 2 - 4 * dkx);
                    pSV.fY3d = RX - dkx * 2 - Rnd.Get(RX * 2 - 4 * dkx);
                    pSV.fZ3d = RY - dky * 2 - Rnd.Get(RY * 2 - 4 * dky);

                    fD2 = (float)Math.Sqrt(pSV.fX3d * pSV.fX3d + pSV.fY3d * pSV.fY3d);
                    fD = (float)Math.Sqrt(fD2 * fD2 + pSV.fZ3d * pSV.fZ3d);
                }
                while (fD > RX || fD2 < RX / 50);
            ...
            for (int i = 0; i < m_iLocationsCount; i++)
            {
                //взяли случайную точку на прямоугольнике
                //float fX = RX - dkx * 2 - Rnd.Get(RX * 2 - 4 * dkx);
                //float fY = RY - dky * 2 - Rnd.Get(RY * 2 - 4 * dky); 

                //перевели (x,y) в широту и долготу, как если бы на нашем прямоугольнике была равновеликая цилиндрическая проекция (http://mathworld.wolfram.com/CylindricalEqual-AreaProjection.html)
                //такая проекция характеризуется тем, что фигуры имеющие равные площади на сфере, будут иметь равные площади и в проекции.
                //нам это как раз нужно для того, чтобы распределение точек на сфере было по возможности равномерным.
                //float fLongitude = (float)(fX / RX);
                //float fLatitude = (float)(Math.Asin(fY / RY));

                float fLongitude = (float)Math.Atan2(pSphere[i].fX3d, pSphere[i].fY3d);
                float fLatitude = (float)Math.Atan2(pSphere[i].fZ3d, Math.Sqrt(pSphere[i].fX3d * pSphere[i].fX3d + pSphere[i].fY3d * pSphere[i].fY3d));

                //теперь широту и долготу переводим опять в (x, y), но на этот раз используем равноугольную меркаторскую проекцию
                //для того, чтобы сохранить взимное расположение точек
                float fX = fLongitude * RX;
                //float fY = (float)(0.5 * Math.Log((1 + Math.Sin(fLatitude)) / (1 - Math.Sin(fLatitude)))) * RY;
                float fY = fLatitude * RY;

                LOC pEquatorLocation = new LOC();

                if (fLatitude < Math.PI / 4 && fLatitude > -Math.PI / 4)
                {
                    pEquatorLocation.Create(iCounter++, fX, fY, 0);
                    cEquatorLocations.Add(pEquatorLocation);

                    LOC pHemisphereLocation = new LOC();
                    if (fLatitude > 0)
                        pHemisphereLocation.Create(iCounter++, (float)(RY * (Math.PI / 2 - fLatitude) * Math.Cos(fLongitude)), (float)(RY * (Math.PI / 2 - fLatitude) * Math.Sin(fLongitude)), 0, null, pEquatorLocation);
                    else
                        pHemisphereLocation.Create(iCounter++, (float)(RY * (Math.PI / 2 + fLatitude) * Math.Cos(fLongitude)), (float)(RY * (Math.PI / 2 + fLatitude) * Math.Sin(fLongitude)), 0, null, pEquatorLocation);

                    pHemisphereLocation.m_bBorder = true;
                    pHemisphereLocation.m_bGhost = true;

                    if (fLatitude > 0)
                        cNorthLocations.Add(pHemisphereLocation);
                    else
                        cSouthLocations.Add(pHemisphereLocation);
                }
                else
                {
                    LOC pHemisphereLocation = new LOC();
                    if (fLatitude > 0)
                    {
                        pHemisphereLocation.Create(iCounter++, (float)(RY * (Math.PI / 2 - fLatitude) * Math.Cos(fLongitude)), (float)(RY * (Math.PI / 2 - fLatitude) * Math.Sin(fLongitude)), 0);
                        cNorthLocations.Add(pHemisphereLocation);
                    }
                    else
                    {
                    ...
                    pEquatorLocation.Create(iCounter++, fX, fY, 0, null, pHemisphereLocation);

                    pEquatorLocation.m_bBorder = true;
                    pEquatorLocation.m_bGhost = true;

                    cEquatorLocations.Add(pEquatorLocation);
                }

                //теперь завернём нашу экваториальную плоскость в цилиндр, добавив справа и слева "призраков"
                LOC pEquatorGhostLocation = new LOC();

                if (pEquatorLocation.X > 0)
                    pEquatorGhostLocation.Create(iCounter++, pEquatorLocation.X - RX * 2, pEquatorLocation.Y, 0, pEquatorLocation, null);
                else
                    pEquatorGhostLocation.Create(iCounter++, pEquatorLocation.X + RX * 2, pEquatorLocation.Y, 0, pEquatorLocation, null);

                pEquatorGhostLocation.m_bBorder = true;
                pEquatorGhostLocation.m_bGhost = true;

                cEquatorLocations.Add(pEquatorGhostLocation);
            }

            m_aLocations = cEquatorLocations.ToArray();
            m_aNorthHemisphereLocations = cNorthLocations.ToArray();
            m_aSouthHemisphereLocations = cSouthLocations.ToArray();
        }
