...
namespace TestCubePlanet
{
    ...
    public struct Region8
    {
        ...
        public bool Intersects(BoundingFrustum pFrustum, Matrix pWorld)
        {
            ...
            Vector3 bfrW = Vector3.Transform(bfr, pWorld);

            float distance1 = Vector3.Dot(Vector3.Normalize(pFrustum.Near.Normal), bblW) + pFrustum.Near.D;
            float distance2 = Vector3.Dot(Vector3.Normalize(pFrustum.Near.Normal), bbrW) + pFrustum.Near.D;
            float distance3 = Vector3.Dot(Vector3.Normalize(pFrustum.Near.Normal), bflW) + pFrustum.Near.D;
            float distance4 = Vector3.Dot(Vector3.Normalize(pFrustum.Near.Normal), bfrW) + pFrustum.Near.D;
