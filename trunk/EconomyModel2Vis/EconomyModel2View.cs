...
{
    public partial class EconomyModel2View : UserControl
    {
        ...
        public void DrawMap()
        {
            ...
            Brush fill = Brushes.DarkGray;
            gr.FillRectangle(fill, 0, 0, ClientRectangle.Width, ClientRectangle.Height);

            if (m_pUniverse == null)
                return;

            if (m_pUniverse.Worlds == null)
                return;

            if (m_pUniverse.Worlds.Count == 0)
                return;
        ...
        private void EconomyModelView_MouseMove(object sender, MouseEventArgs e)
        {
            ...
            float fkX = ClientRectangle.Width / (m_pUniverse.Width+1);
            float fkY = ClientRectangle.Height / (m_pUniverse.Height+1);

            int iX = (int)(0.5+(e.X - fkX) / fkX);
            int iY = (int)(0.5+(e.Y - fkY) / fkY);

            CWorld pSelected = m_pSelectedWorld;

            foreach (CWorld pWorld in m_pUniverse.Worlds)
            {
                if (pWorld.X == iX && pWorld.Y == iY)
                    pSelected = pWorld;
            }
        ...
        private void DrawWorld(CWorld pWorld, Brush lightUp)
        {
            ...
            int iDiameter = 2 + pWorld.Population / 2;
            ...
            gr.FillEllipse(lightUp, fkX + pWorld.X * fkX - iDiameter / 2 - 1, fkY + pWorld.Y * fkY - iDiameter / 2 - 1, iDiameter + 2, iDiameter + 2);

            foreach (CTradingRoute pRoute in pWorld.Spaceport.Routes)
            {
                if (pRoute.Byer == pWorld)
                {
                    Pen _ligthUp = new Pen(lightUp);
                    gr.DrawLine(Pens.Aqua, fkX + pWorld.X * fkX, fkY + pWorld.Y * fkY,
                        fkX + pRoute.Seller.X * fkX, fkY + pRoute.Seller.Y * fkY);
                }
            }

            Brush marking = GetWorldBrush(pWorld);
            gr.FillEllipse(marking, fkX + pWorld.X * fkX - iDiameter / 2, fkY + pWorld.Y * fkY - iDiameter / 2, iDiameter, iDiameter);
            ...
            //    if (iInnerD < 2)
            //        iInnerD = 2;
            //    gr.FillEllipse(Brushes.Navy, fkX + pWorld.X * fkX - iInnerD / 2, fkY + pWorld.Y * fkY - iInnerD / 2, iInnerD, iInnerD);
            //}
        }
