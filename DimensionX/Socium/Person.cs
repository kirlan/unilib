...
namespace Socium
{
    public class Person
    {
        ...
        public Phenotype m_pFamilyFenotypeM;
        public Phenotype m_pFamilyFenotypeF;
        public Phenotype m_pFenotype;
        ...
        {
            var pOriginCulture = pRelative != null && pRelative.m_eGender == m_eGender ? pRelative.m_pCulture : m_pEstate.m_cCulture[m_eGender];

            var pCulture = new Mentality(pOriginCulture.m_pMentality, true);
            var iCultureLevel = pOriginCulture.m_iProgressLevel;
            ...
            if (iCultureLevel < m_pState.m_cCulture[m_eGender].m_iProgressLevel - 1)
                iCultureLevel = m_pState.m_cCulture[m_eGender].m_iProgressLevel - 1;
            ...
            var pCustoms = new Customs(pOriginCulture.m_pCustoms, Customs.Mutation.Possible);

            m_pCulture = new Culture(pCulture, iCultureLevel, pCustoms);

            m_pFamilyFenotypeM = pRelative == null ? (Phenotype)m_pNation.m_pFenotypeM.MutateFamily() : pRelative.m_pFamilyFenotypeM;
            m_pFamilyFenotypeF = pRelative == null ? (Phenotype)m_pNation.m_pFenotypeF.MutateFamily() : pRelative.m_pFamilyFenotypeF;
            if (m_eGender == Gender.Male)
                m_pFenotype = (Phenotype)m_pFamilyFenotypeM.MutateIndividual();
            else
                m_pFenotype = (Phenotype)m_pFamilyFenotypeF.MutateIndividual();
        ...
        public void ApplyInjuries()
        {
            ...
            {
                if (Rnd.Chances(1 + (int)m_pProfession.m_cSkills[Skill.Body], 8 + m_pState.m_iInfrastructureLevel * 2))
                {
                ...
                if (Rnd.Chances(1 + (int)m_pProfession.m_cSkills[Skill.Mind] + (int)m_pProfession.m_cSkills[Skill.Charm], 8 + m_pState.m_iInfrastructureLevel * 2))
                {
        ...
        public Person(World pWorld, LocationX pHomeLocation, Building pBuilding, bool bOwner)
        {
            ...
            m_pNation = m_pState.m_pTitularNation;
            ...
            else
            {
                ...
                if (m_pState.m_pStateModel.m_bDinasty)// && Rnd.OneChanceFrom(3))
                    pRelative = pWorld.GetPossibleRelative(m_pNation, m_pState.m_iInfrastructureLevel);
            ...
            foreach (var pEstate in m_pState.m_cEstates)
            {
        ...
        public Person(World pWorld, LocationX pPreferredHome)
        {
            ...
            if (pRelative != null)
            {
                ...
                {
                    if (!Rnd.Chances(pRelative.m_pState.m_iSocialEquality, 8))
                        eEstate = pRelative.m_pEstate.m_ePosition;
            ...
            m_pEstate = m_pState.m_cEstates[eEstate];
            ...
            if (pRelative != null && IsFamily(m_cRelations[pRelative]) && pRelative.CouldInviteNewDwellers())
            {
                ...
                else
                {
                    ...
                    if (pRelative.m_pBuilding.m_pInfo == pRelative.m_pState.m_pStateModel.m_pStateCapital.m_pMainBuilding)
                    {
                        foreach (Building pBuilding in m_pHomeLocation.m_pSettlement.m_cBuildings)
                        {
                            if (pBuilding.m_pInfo == m_pState.m_pStateModel.m_pStateCapital.m_pMainBuilding ||
                                pBuilding.m_pInfo == m_pState.m_pStateModel.m_pProvinceCapital.m_pMainBuilding)
                            {
                                m_pBuilding = pBuilding;
                                break;
                            }
        ...
        public void FixSubordination(World pWorld)
        {
            ...
            {
                foreach (Building pBuilding in m_pHomeLocation.m_pSettlement.m_cBuildings)
                    if (pBuilding.m_pInfo == m_pState.m_pStateModel.m_pProvinceCapital.m_pMainBuilding ||
                        pBuilding.m_pInfo == m_pState.m_pStateModel.m_pStateCapital.m_pMainBuilding)
                        foreach (Person pDweller in pBuilding.m_cPersons)
            ...
            if (m_pBuilding.m_pInfo == m_pState.m_pStateModel.m_pProvinceCapital.m_pMainBuilding &&
                m_pProfession.m_bMaster)
            {
                foreach (LocationX pLand in m_pState.Settlements)
                {
                    foreach (Building pBuilding in pLand.m_pSettlement.m_cBuildings)
                    {
                        if ((pBuilding.m_pInfo == m_pState.m_pStateModel.m_pProvinceCapital.m_pMainBuilding && Rnd.OneChanceFrom(2)) ||
                            pBuilding.m_pInfo == m_pState.m_pStateModel.m_pStateCapital.m_pMainBuilding)
                        {
                            foreach (Person pDweller in pBuilding.m_cPersons)
                            {
            ...
            if (m_pBuilding.m_pInfo == m_pState.m_pStateModel.m_pStateCapital.m_pMainBuilding &&
                m_pProfession.m_bMaster)
            {
                foreach (State pState in pWorld.m_aStates)
                {
        ...
        private Relation PossibleRelation(Person pPretender)
        {
            ...
            { 
                switch(m_pFenotype.ValueOf<LifeCycleGenetix>().m_eBirthRate)
                {
        ...
        public bool IsRuler(bool bMinor)
        {
            ...
            if (m_pState.m_pStateModel.m_pStateCapital.m_pMainBuilding.m_pOwnerProfession == m_pProfession)
                return true;

            if (bMinor && m_pState.m_pStateModel.m_pProvinceCapital.m_pMainBuilding.m_pOwnerProfession == m_pProfession)
                return true;
        ...
        public bool IsInGovernment()
        {
            ...
            if (//m_pHome.Owner.m_pInfo.m_pStateCapital.m_pMainBuilding.m_eOwnership == FamilyOwnership.Full && 
                m_pState.m_pStateModel.m_pStateCapital.m_pMainBuilding == m_pBuilding.m_pInfo)
                return true;
        ...
        public int GetInfluence(bool bCombined)
        {
            ...
            switch (m_pState.m_iInfrastructureLevel)
            {
        ...
        public int GetFenotypeDifference(Person pOpponent, ref string sPositiveReasons, ref string sNegativeReasons)
        {
            ...
            Phenotype pNation = pOpponent.Gender == Gender.Male ? m_pNation.m_pFenotypeM : m_pNation.m_pFenotypeF;
            Phenotype pOpponents = pOpponent.m_pFenotype;
        ...
        public int GetCustomsDifference(Person pOpponent, ref string sPositiveReasons, ref string sNegativeReasons)
        {
            ...
            Customs pMine = m_pCulture.m_pCustoms;
        ...
        public string GetFenotypeComparsion(Phenotype pOriginal)
        {
            ...
            if (!pOriginal.HasIdentical<HeadGenetix>(m_pFenotype) ||
                !pOriginal.HasIdentical<ArmsGenetix>(m_pFenotype) ||
                !pOriginal.HasIdentical<LegsGenetix>(m_pFenotype) ||
                !pOriginal.HasIdentical<WingsGenetix>(m_pFenotype) ||
                !pOriginal.HasIdentical<TailGenetix>(m_pFenotype))
            {
                if (sResult != "")
                    sResult += " ";
                ...
                if (!pOriginal.HasIdentical<HeadGenetix>(m_pFenotype))
                {
                    sResult += m_pFenotype.ValueOf<HeadGenetix>().GetDescription();
                    bSemicolon = true;
                }

                if (!pOriginal.HasIdentical<ArmsGenetix>(m_pFenotype))
                {
                    if (m_pFenotype.ValueOf<ArmsGenetix>().m_eArmsCount != ArmsCount.None)
                    {
                        if (bSemicolon)
                            sResult += ", ";
                        sResult += m_pFenotype.ValueOf<ArmsGenetix>().GetDescription();
                ...
                if (!pOriginal.HasIdentical<LegsGenetix>(m_pFenotype))
                {
                    if (bSemicolon)
                        sResult += ", ";
                    sResult += m_pFenotype.ValueOf<LegsGenetix>().GetDescription();

                    bSemicolon = true;
                }

                if (!pOriginal.HasIdentical<WingsGenetix>(m_pFenotype))
                {
                    if (m_pFenotype.ValueOf<WingsGenetix>().m_eWingsCount != WingsCount.None)
                    {
                        if (bSemicolon)
                            sResult += ", ";
                        sResult += m_pFenotype.ValueOf<WingsGenetix>().GetDescription();
                ...
                if (!pOriginal.HasIdentical<TailGenetix>(m_pFenotype))
                {
                    if (m_pFenotype.ValueOf<TailGenetix>().m_eTailLength != TailLength.None)
                    {
                        if (bSemicolon)
                            sResult += " and ";
                        sResult += m_pFenotype.ValueOf<TailGenetix>().GetDescription();
                    }
                    else
                    {
                        if (bSemicolon)
            ...
            if (!pOriginal.HasIdentical<BodyGenetix>(m_pFenotype) ||
                !pOriginal.HasIdentical<HideGenetix>(m_pFenotype))
            {
                if (sResult != "")
                    sResult += " ";
                ...
                if (!pOriginal.HasIdentical<BodyGenetix>(m_pFenotype))
                {
                    sResult += (m_eGender == Gender.Male ? "He is a " : "She is a ") + m_pFenotype.ValueOf<BodyGenetix>().GetDescription(m_eGender);
                    bSemicolon = true;
                }
                else
                    sResult += (m_eGender == Gender.Male ? "He " : "She ") + "has ";

                if (!pOriginal.HasIdentical<HideGenetix>(m_pFenotype))
                {
                    if (bSemicolon)
                        sResult += " with ";
                    sResult += m_pFenotype.ValueOf<HideGenetix>().GetDescription();
                }

                sResult += ".";

                sResult += m_pFenotype.ValueOf<BodyGenetix>().GetDescription2(m_eGender);
            }

            if (!pOriginal.HasIdentical<EyesGenetix>(m_pFenotype) ||
                !pOriginal.HasIdentical<EarsGenetix>(m_pFenotype) ||
                !pOriginal.HasIdentical<FaceGenetix>(m_pFenotype))
            {
                if (sResult != "")
                    sResult += " ";
                    ...
                    !pOriginal.HasIdentical<EarsGenetix>(m_pFenotype))
                {
                    ...
                    if (!pOriginal.HasIdentical<EyesGenetix>(m_pFenotype))
                    {
                        sResult += m_pFenotype.ValueOf<EyesGenetix>().GetDescription();
                        bSemicolon = true;
                    }

                    if (!pOriginal.HasIdentical<EarsGenetix>(m_pFenotype))
                    {
                        if (bSemicolon)
                            sResult += " and ";

                        sResult += m_pFenotype.ValueOf<EarsGenetix>().GetDescription() + " of ";
                    }
                    else
                    {
                        if (bSemicolon && !pOriginal.HasIdentical<FaceGenetix>(m_pFenotype))
                            sResult += " at ";
                    }
                }
                else
                ...
                if (!pOriginal.HasIdentical<FaceGenetix>(m_pFenotype))
                    sResult += m_pFenotype.ValueOf<FaceGenetix>().GetDescription();
                else
                    if (!pOriginal.HasIdentical<EarsGenetix>(m_pFenotype))
            ...
            //    (m_eGender == CPerson.Gender.Female && (pOriginal.m_pHairs.m_eHairsF != m_pFenotype.m_pHairs.m_eHairsF || pOriginal.m_pHairs.m_eBeardF != m_pFenotype.m_pHairs.m_eBeardF)))
            {
                ...
                if (m_pFenotype.ValueOf<HairsGenetix>().GetDescription() != "")
                    sResult += m_pFenotype.ValueOf<HairsGenetix>().GetDescription(m_eGender);
                else
        ...
        public string GetCustomsDescription()
        {
            ...
            {
                bool bFirst2 = true;
                foreach (Customs.BodyModificationsTypes eMod in m_pCulture.m_pCustoms.m_cMandatoryModifications)
                {
                    if (bFirst2)
                        sResult += "has ";
                    else
        ...
        public int CalcHostility(Person pOpponent, out string sReasons, bool bFirstLook)
        {
            ...
            if (!bFirstLook)
            {
                ...
                if (pOpponent.m_pState.m_iInfrastructureLevel > m_pState.m_iInfrastructureLevel + 1)
                {
                ...
                if (IsSlave())
                {
                    if (pOpponent.m_pState.m_iSocialEquality == 0 && pOpponent.m_pEstate.IsMiddleUp())
                    {
                        iHostility += 2;
                ...
                if (m_pState.m_iSocialEquality == 0 && m_pEstate.IsMiddleUp())
                {
                    if (pOpponent.IsSlave())
                    {
                        iHostility += 2;
                ...
                if (m_pState.m_iSocialEquality > 1)
                {
                    if (pOpponent.m_pState.m_iSocialEquality == 0 && pOpponent.m_pEstate.IsMiddleUp())
                    {
                        iHostility++;
                ...
                float iCultureDifference = m_pCulture.m_pMentality.GetDifference(pOpponent.m_pCulture.m_pMentality, m_pCulture.m_iProgressLevel, pOpponent.m_pCulture.m_iProgressLevel);
        ...
        public void FixHealth()
        {
            foreach (var pRelation in m_cRelations)
            {
                if (pRelation.Value == Relation.AlterEgo ||
                    pRelation.Value == Relation.PresentLifeOf)
                {
                    foreach (Customs.BodyModificationsTypes eMod in pRelation.Key.m_pCulture.m_pCustoms.m_cMandatoryModifications)
                        if (!m_pCulture.m_pCustoms.m_cMandatoryModifications.Contains(eMod))
                            m_pCulture.m_pCustoms.m_cMandatoryModifications.Add(eMod);
        ...
        private void SetShortName()
        {
            ...
            if (m_pProfession != ProfessionInfo.Nobody)
            {
                ...
                if (m_pBuilding.m_pInfo == m_pState.m_pStateModel.m_pStateCapital.m_pMainBuilding)
                    sShortName.AppendFormat(" of {0} {1}", m_pState.m_sName, m_pState.m_pStateModel.m_sName);
                else if (m_pBuilding.m_pInfo == m_pState.m_pStateModel.m_pProvinceCapital.m_pMainBuilding)
                    sShortName.AppendFormat(" of {0} {1} ({2} {3})", m_pHomeLocation.m_pSettlement.m_pInfo.m_sName.ToLower(), m_pHomeLocation.m_pSettlement.m_sName, m_pState.m_sName, m_pState.m_pStateModel.m_sName);
                else
                    sShortName.AppendFormat(" from {0} {1} ({2} {3})", m_pHomeLocation.m_pSettlement.m_pInfo.m_sName.ToLower(), m_pHomeLocation.m_pSettlement.m_sName, m_pState.m_sName, m_pState.m_pStateModel.m_sName);
            }
