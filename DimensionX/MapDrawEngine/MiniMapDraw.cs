    ...
    public partial class MiniMapDraw : UserControl
    {
        ...
        private PointF[][] BuildPath(List<Line> cFirstLines)
        {
            ...
            foreach (Line pFirstLine in cFirstLines)
            {
                ...
                if (m_pMasterMap.m_pWorld.m_pPlanet.m_bCycled && bCross)
                {
                    //определяем, на западе или на востоке будем строить отражение
                    if (pFirstLine.m_pPoint1.X > 0)
                        cPath.Add(BuildBorder(pFirstLine, -m_pMasterMap.m_pWorld.m_pPlanet.RX * 2, out bCross));
                    else
                        cPath.Add(BuildBorder(pFirstLine, m_pMasterMap.m_pWorld.m_pPlanet.RX * 2, out bCross));
                }
            }
        ...
        private PointF[] BuildBorder(Line pFirstLine, float fShift, out bool bCross)
        {
            ...
            do
            {
                ...
                float fDX = fShift;
                if (Math.Abs(fLastPointX - pLine.m_pPoint2.X - fShift) > m_pMasterMap.m_pWorld.m_pPlanet.RX)
                {
                    ...
                    //не было разрыва
                    fDX += fLastPointX < fShift ? -m_pMasterMap.m_pWorld.m_pPlanet.RX * 2 : m_pMasterMap.m_pWorld.m_pPlanet.RX * 2;
                    bCross = true;
                }

                if (pLine.m_pPoint2.X > m_pMasterMap.m_pWorld.m_pPlanet.RX ||
                    pLine.m_pPoint2.X < -m_pMasterMap.m_pWorld.m_pPlanet.RX)
                    bCross = true;
        ...
        {
            return new PointF(m_pMasterMap.m_pWorld.m_pPlanet.RX + pPoint.X + fDX, m_pMasterMap.m_pWorld.m_pPlanet.RY + pPoint.Y);
        }
        ...
        private void CreateCanvas()
        {
            ...
            float fK = 1;
            if (m_pMasterMap != null && m_pMasterMap.m_pWorld != null)
                fK = (float)m_pMasterMap.m_pWorld.m_pPlanet.RY / m_pMasterMap.m_pWorld.m_pPlanet.RX;
            ...
            if (m_pMasterMap.m_pWorld != null)
                m_fActualScale = (float)(m_iScaledMapWidth) / (m_pMasterMap.m_pWorld.m_pPlanet.RX * 2);
        ...
        public void Draw()
        {
            ...
            if (m_pMasterMap == null || m_pMasterMap.m_pWorld == null || m_pMasterMap.m_pWorld.m_pPlanet.m_aLocations.Length == 0)
                return;
        ...
        public void SetPan(int iX, int iY)
        {
            ...
            if (m_pMasterMap.m_pWorld.m_pPlanet.m_bCycled)
                m_pDrawFrame.X = iX;
            else
